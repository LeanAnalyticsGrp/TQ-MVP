	...
	var WaitlistCtrl = function($scope, $firebase, FIREBASE_URL, restaurant, Restaurant, Guests, SMS, Notification, MetaInformation){
		...
		var restaurantId = restaurant.obj.$id;
		 ...
		 for(var i=0;i<this.tempparties.length;i++){
			 var phone = this.tempparties[i].phone.replace( /\d{4}/, '****');
			 //console.log("Phone22: "+ angular.toJson(phone));
			 this.tempparties[i].phone = phone;
			 //console.log("Phone23: "+ this.tempparties[i].phone);
		  }
